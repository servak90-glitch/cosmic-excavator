...
const SkillsView: React.FC<SkillsViewProps> = ({ state, onUpgradeSkill }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#050505] p-4 relative overflow-hidden">
      {/* Background Grid */}
      ...
      {/* HEADER */}
      <div className="relative z-10 border-b border-zinc-800 pb-4 mb-4 flex justify-between items-end">
        <div>
           ...
           <div className="text-2xl font-mono text-cyan-400 font-bold drop-shadow-[0_0_5px_rgba(34,211,238,0.5)]">
             {Math.floor(state.xp).toLocaleString()} UNITS
           </div>
        </div>
        <div className="text-right hidden md:block">
           <div className="text-[10px] text-zinc-600 font-mono">СИНХРОНИЗАЦИЯ: {(state.level * 100).toFixed(0)}%</div>
            ...
            <div key={cat.id} className="relative">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ml-2 border-l border-dashed border-zinc-800 pl-4 py-2">
                  {categorySkills.map(skill => {
                    ...
                    if (skill.requiredDepth && state.depth < skill.requiredDepth) {
                      locked = true;
                      lockReason = `ТРЕБУЕТСЯ ГЛУБИНА ${skill.requiredDepth}м`;
                    }
                    if (skill.requiredParent) {
                      const parentLevel = state.skillLevels[skill.requiredParent] || 0;
                      if (parentLevel < 1) {
                         locked = true;
